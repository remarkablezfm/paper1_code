...
def try_project_vehicle_points_to_image(
    ...
    """
    Best-effort projection using py_camera_model_ops if available.
    ...
    # points: [N,3]
    pts = tf.convert_to_tensor(points_xyz_vehicle.astype(np.float32))

    # Some ops variants:
    #  - vehicle_to_image(points, camera_calibration, vehicle_pose) OR without vehicle_pose
    #  - world_to_image(world_points, camera_calibration, vehicle_pose)
    # We only have vehicle-frame points reliably, so prefer vehicle_to_image.
    try:
        ...
        if callable(fn):
            try:
                ...
                if vehicle_pose_4x4 is not None:
                    pose = tf.convert_to_tensor(vehicle_pose_4x4.astype(np.float32))
                    uv = fn(pts, cam_calib_proto, pose)
                else:
                    uv = fn(pts, cam_calib_proto)
                uv = uv.numpy()
                return uv[:, :2], "ok_vehicle_to_image"
            except TypeError:
                # maybe signature differs
                pass

        # fallback: world_to_image if provided; treat vehicle points as world (not correct but sometimes works if pose=I)
        fn2 = getattr(py_camera_model_ops, "world_to_image", None)
        if callable(fn2):
            pose = tf.convert_to_tensor(vehicle_pose_4x4.astype(np.float32)) if vehicle_pose_4x4 is not None else None
            if pose is None:
                # identity pose
                pose = tf.eye(4, dtype=tf.float32)
            uv = fn2(pts, cam_calib_proto, pose).numpy()
            return uv[:, :2], "ok_world_to_image_fallback"
    except Exception as e:
        return None, f"projection_error:{type(e).__name__}:{str(e)[:120]}"
    ...
    for _, r in seg_rows.iterrows():
        ...
        def overlay_on_cam(cam_id_str: str, out_name: str) -> None:
            ...
            for ti, (tname, tdf) in enumerate(traj_to_draw):
                ...
                uv, status = try_project_vehicle_points_to_image(pts, calib, pose_4x4)
